...
# Add auto shutdown schedule if enabled for VM
resource "azurerm_dev_test_global_vm_shutdown_schedule" "lvm_shutdown" {
  for_each = {for v in var.lvms : v.name => v if v.auto_shutdown_enable}
...
# Add AMA agent if enabled for VM
resource "azurerm_virtual_machine_extension" "lvm_ama" {
  for_each = {for v in var.lvms : v.name => v if v.ama_enable}
  name = "ama-${each.value.name}"
  virtual_machine_id = try(azurerm_linux_virtual_machine.lvm[each.key].id,null)
